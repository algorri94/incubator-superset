...
class NVD3Vis extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    this.addTotalBarValues = this.addTotalBarValues.bind(this);
  ...
  filterData(e) {
    ...
    if(datum.filter) {
      if(selectedValues.hasOwnProperty(datum.filter) && !selectedValues[datum.filter].includes(datum.x)) {
        selectedValues[datum.filter].push(datum.x);
      } else if (selectedValues.hasOwnProperty(datum.filter) && selectedValues[datum.filter].includes(datum.x)) { 
        var index = selectedValues[datum.filter].indexOf(datum.x);
        ...
        selectedValues[datum.filter] = [datum.x];
      }
  ...
  addTotalBarValues(svg, chart, data, stacked, axisFormat) {
  ...
  getMaxLabelSize(container, axisClass) {
  ...
  drawGraph() {
    ...
    if (this.props.data) {
      if (Array.isArray(this.props.data)) {
          data = this.props.data.map(x => ({
              ...x, key: this.formatLabel(x.key, this.props.verboseMap),
          }));
      } else {
    ...
    let svg = d3.select(this.props.selector).select('svg');
    if (svg.empty()) {
      svg = d3.select(this.props.selector).append('svg');
    }
    ...
    switch (vizType) {
      ...
      case 'bar':
        ...
        if (fd.show_bar_value) {
          setTimeout(function () {
            this.addTotalBarValues(svg, chart, data, stacked, fd.y_axis_format);
          }, animationTime);
        }
        break;
    ...
    if (isTruthy(fd.show_brush) && isTruthy(fd.send_time_range)) {
      chart.focus.dispatch.on('brush', (event) => {
        ...
        event.brush.on('brushend', () => this.props.addFilter('__time_range', timeRange, false, true));
      });
    }
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
      ...
      const maxYAxisLabelWidth = chart.yAxis2 ? this.getMaxLabelSize(this.props.container, 'nv-y1')
                                              : this.getMaxLabelSize(this.props.container, 'nv-y');
      const maxXAxisLabelHeight = this.getMaxLabelSize(this.props.container, 'nv-x');
      ...
      if (['dual_line', 'line_multi'].indexOf(vizType) >= 0) {
        const maxYAxis2LabelWidth = this.getMaxLabelSize(this.props.container, 'nv-y2');
      ...
      const annotationLayers = (this.props.formData.annotation_layers || []).filter(x => x.show);
      if (isTimeSeries && annotationLayers && this.props.annotationData) {
        ...
        bushel.concat((this.props.annotationData[a.name] || []).map((series) => {
          if (!series) {
            return {};
          }
      ...
      $(window).scroll(throttle(this.hideTooltips, 250));
      ...
      if (isTimeSeries && annotationLayers) {
        ...
        if (this.props.annotationData) {
          ...
          annotationLayers.filter(x => (
            ...
            this.props.annotationData && this.props.annotationData[x.name]
          )).forEach((config, index) => {
            const e = applyNativeColumns(config);
            // Add event annotation layer
            const annotations = d3.select(this.props.selector).select('.nv-wrap').append('g')
              .attr('class', `nv-event-annotation-layer-${index}`);
            const aColor = e.color || getColorFromScheme(e.name, fd.color_scheme);

            const tip = tipFactory(e);
            const records = (this.props.annotationData[e.name].records || []).map((r) => {
    ...
    switch (vizType) {
      case 'pie':
        chart.pie.dispatch.on("elementClick", filterData);
        break;
      case 'bar':
      case 'dist_bar':
        chart.multibar.dispatch.on("elementClick", filterData);
        break;
    }
    this.wrapTooltip(chart, this.props.container);
    return chart;
  }
...
NVD3Vis.defaultProps = defaultProps;

function adaptor(slice, payload) {
  ReactDOM.render(
    <NVD3Vis
      verboseMap={slice.datasource.verbose_map}
      width={slice.width}
